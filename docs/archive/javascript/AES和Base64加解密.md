...
const keyString = "xxxxxx"
let key = CryptoJS.SHA256(keyString).toString()
...
function Decrypt(word) {
  var wordBase = CryptoJS.enc.Base64.parse(word).toString()
  console.log(wordBase)
  const wordHash = CryptoJS.format.Hex.parse(wordBase)
  console.log("wordHash", wordHash)

  var dec = CryptoJS.AES.decrypt(wordHash, key, {
    iv: iv,
