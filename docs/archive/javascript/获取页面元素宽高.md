...
this.$nextTick(() => {
    ...
    */
    ...
    document.documentElement.scrollTop =
        document.getElementsByClassName('current')[0].offsetTop + 5
    window.pageYOffset =
        document.getElementsByClassName('current')[0].offsetTop + 5
    document.body.scrollTop =
        document.getElementsByClassName('current')[0].offsetTop + 5
...
isElementInViewport(el) {
    ...
    let isEleInview = rect.top >= 0 && rect.left >= 0 && rect.bottom <= (window.innerHeight || document.documentElement.clientHeight) && rect.right <= (window.innerWidth || document.documentElement.clientWidth)
    console.log('看到', isEleInview)
    if (!isEleInview) {
...
浏览器当前窗口文档 body 的高度： $(document.body).height();
...
某个元素的上边界到 body 最顶部的距离：obj.offset().top;（在元素的包含元素不含滚动条的情况下）
某个元素的左边界到 body 最左边的距离：obj.offset().left;（在元素的包含元素不含滚动条的情况下）
